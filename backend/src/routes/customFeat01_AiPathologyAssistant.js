...
// Upload slide image; AI auto-segments tissue regions, detects abnormalities, grades severity; generates structured report with confidence scores
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPathologySlideAnalyzer / cf-ai-pathology-assistant',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_ai_pathology_assistant`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'AI pathology assistant'.
Context: Upload slide image; AI auto-segments tissue regions, detects abnormalities, grades severity; generates structured report with confidence scores
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-ai-pathology-assistant/run error:', err);
