...
// Auto-submit de-identified cases to cancer/tissue registries; contribute to research databases
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPathologySlideAnalyzer / cf-registry-integration',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_registry_integration`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Registry integration'.
Context: Auto-submit de-identified cases to cancer/tissue registries; contribute to research databases
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-registry-integration/run error:', err);
