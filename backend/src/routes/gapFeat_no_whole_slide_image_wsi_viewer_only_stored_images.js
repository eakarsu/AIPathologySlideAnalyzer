...
// Gap Feature: No whole slide image (WSI) viewer (only stored images)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPathologySlideAnalyzer / gap-no-whole-slide-image-wsi-viewer-only-stored-images',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No whole slide image (WSI) viewer (only stored images)'.\nProject: AIPathologySlideAnalyzer.\nContext: No whole slide image (WSI) viewer (only stored images)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-whole-slide-image-wsi-viewer-only-stored-images', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-whole-slide-image-wsi-viewer-only-stored-images', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
