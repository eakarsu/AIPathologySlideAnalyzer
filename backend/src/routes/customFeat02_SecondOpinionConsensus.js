...
// Route slide to multiple AI models simultaneously; display agreement/disagreement and explain differences
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPathologySlideAnalyzer / cf-second-opinion-consensus',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_second_opinion_consensus`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Second opinion consensus'.
Context: Route slide to multiple AI models simultaneously; display agreement/disagreement and explain differences
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-second-opinion-consensus/run error:', err);
