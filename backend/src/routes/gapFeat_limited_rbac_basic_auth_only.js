...
// Gap Feature: Limited RBAC (basic auth only)
// Limited RBAC (basic auth only)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPathologySlideAnalyzer / gap-limited-rbac-basic-auth-only',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited RBAC (basic auth only)'.\nProject: AIPathologySlideAnalyzer.\nContext: Limited RBAC (basic auth only)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-rbac-basic-auth-only', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-rbac-basic-auth-only', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
