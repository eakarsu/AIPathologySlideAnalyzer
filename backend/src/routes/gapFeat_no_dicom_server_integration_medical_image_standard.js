...
// Gap Feature: No DICOM server integration (medical image standard)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPathologySlideAnalyzer / gap-no-dicom-server-integration-medical-image-standard',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No DICOM server integration (medical image standard)'.\nProject: AIPathologySlideAnalyzer.\nContext: No DICOM server integration (medical image standard)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-dicom-server-integration-medical-image-standard', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-dicom-server-integration-medical-image-standard', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
