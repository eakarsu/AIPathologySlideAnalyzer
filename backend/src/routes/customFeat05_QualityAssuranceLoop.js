...
// AI flags low-confidence predictions; routes to senior pathologist for review; learns from corrections
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPathologySlideAnalyzer / cf-quality-assurance-loop',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_quality_assurance_loop`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Quality assurance loop'.
Context: AI flags low-confidence predictions; routes to senior pathologist for review; learns from corrections
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-quality-assurance-loop/run error:', err);
