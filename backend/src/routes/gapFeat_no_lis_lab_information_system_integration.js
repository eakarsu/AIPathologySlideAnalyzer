...
// Gap Feature: No LIS (Lab Information System) integration
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPathologySlideAnalyzer / gap-no-lis-lab-information-system-integration',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No LIS (Lab Information System) integration'.\nProject: AIPathologySlideAnalyzer.\nContext: No LIS (Lab Information System) integration\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-lis-lab-information-system-integration', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-lis-lab-information-system-integration', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
