...
// Overlay AI annotations on slide; highlight areas of interest; allow pathologist to drill down and learn model reasoning
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPathologySlideAnalyzer / cf-educational-mode',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_educational_mode`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Educational mode'.
Context: Overlay AI annotations on slide; highlight areas of interest; allow pathologist to drill down and learn model reasoning
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-educational-mode/run error:', err);
