...
// Gap (Non-AI): Limited RBAC (basic auth only)
// Limited RBAC (basic auth only)
import React, { useState } from 'react';

export default function GapLimitedRbacBasicAuthOnlyPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-rbac-basic-auth-only/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Limited RBAC (basic auth only)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Limited RBAC (basic auth only)</p>
