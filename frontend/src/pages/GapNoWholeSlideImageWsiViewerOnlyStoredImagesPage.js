...
// Gap (Non-AI): No whole slide image (WSI) viewer (only stored images)
// No whole slide image (WSI) viewer (only stored images)
import React, { useState } from 'react';

export default function GapNoWholeSlideImageWsiViewerOnlyStoredImagesPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-whole-slide-image-wsi-viewer-only-stored-images/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No whole slide image (WSI) viewer (only stored images)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No whole slide image (WSI) viewer (only stored images)</p>
