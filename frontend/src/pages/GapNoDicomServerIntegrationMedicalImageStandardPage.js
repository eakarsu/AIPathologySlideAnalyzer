...
// Gap (Non-AI): No DICOM server integration (medical image standard)
// No DICOM server integration (medical image standard)
import React, { useState } from 'react';

export default function GapNoDicomServerIntegrationMedicalImageStandardPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-dicom-server-integration-medical-image-standard/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No DICOM server integration (medical image standard)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No DICOM server integration (medical image standard)</p>
